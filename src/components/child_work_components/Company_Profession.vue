<template>
  <div class="company_container">
    <div class="company_up_box">
      <el-row :gutter="25">
        <el-col :span="4">
          <div class="company_data_title">
            <h3>公司地理分布情况</h3>
          </div>
        </el-col>
      </el-row>
      <el-row :gutter="10">
        <el-col :span="1" style="border-right: 0;">
          <div style="height: 10px">
          </div>
        </el-col>
        <el-col :span="18" style="border-right: 0;">
          <div id="company_location_echart" class="company_location_box">
          </div>
        </el-col>
        <el-col :span="4" style="border-right: 0;">
          <div id="company_location_list_echart" class="company_location_list_box">
          </div>
        </el-col>
        <el-col :span="1" style="border-right: 0;">
          <div style="height: 10px">
          </div>
        </el-col>
      </el-row>
    </div>

    <div class="company_center_box">
      <div class="company_select_box">
        <el-row :gutter="25">
          <el-col :span="5">
            <div class="company_data_title">
              <h3>公司月发布招聘分析</h3>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="25">
          <el-col :span="1" style="border-right: 0;">
            <div style="height: 10px">
            </div>
          </el-col>
          <el-col :span="23">
            <div class="company_time_select_options">
              时间范围：
              <el-date-picker
                v-model="selectValue.timeValue"
                type="monthrange"
                range-separator="至"
                start-placeholder="开始月份"
                end-placeholder="结束月份">
              </el-date-picker>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="25" style="margin-bottom: 0px;">
          <el-col :span="1" style="border-right: 0;">
            <div style="height: 10px">
            </div>
          </el-col>
          <el-col :span="23">
            <div class="company_select_options">
              选择公司：
              <el-select v-model="companyValue" filterable placeholder="输入或选择公司">
                <el-option
                  v-for="item in companyOptions"
                  :key="item.companyId"
                  :label="item.companyName"
                  :value="item.companyId">
                </el-option>
              </el-select>
            </div>
          </el-col>
        </el-row>
        <el-row :gutter="25">
          <el-col :span="1" style="border-right: 0;">
            <div style="height: 10px">
            </div>
          </el-col>
          <el-col :span="23">
            <div class="company_select_options">
              <el-tag
                v-for="tag in tags"
                :key="tag.id"
                closable
                @close="handleClose(tag)">
                {{tag.name}}
              </el-tag>
            </div>
          </el-col>
        </el-row>
      </div>
      <div class="company_new_profession_box">
        <el-row :gutter="20">
          <el-col :span="2" style="border-right: 0;">
            <div style="height: 10px">
            </div>
          </el-col>
          <el-col :span="22">
            <div id="company_new_profession_echart" class="company_new_profession_line_box">
            </div>
          </el-col>
        </el-row>
      </div>
    </div>

    <div class="company_end_box">
      <el-row :gutter="25">
        <el-col :span="5">
          <div class="company_data_title">
            <h3>公司 招聘/简历 排行榜一览</h3>
          </div>
        </el-col>
      </el-row>
      <el-row :gutter="25">
        <el-col :span="8">
          <div id="company_profession_list_echart" class="company_list_box">
          </div>
        </el-col>
        <el-col :span="8">
          <div id="company_resume_list_echart" class="company_list_box">
          </div>
        </el-col>
        <el-col :span="8">
          <div id="company_resume_state_echart" class="company_list_box">
          </div>
        </el-col>
      </el-row>
    </div>
  </div>
</template>

<script>
  export default {
    name: 'company_profession',
    data () {
      return {
        locationList: [],
        companyOptions: [],
        companyValue: '',
        tags: [],
        selectValue: {
          companyId: [],
          timeValue: []
        },
        professionChange: {
          time: [],
          profession: []
        },
        professionList: {
          companyName:[],
          number:[]
        },
        resumeList: {
          companyName:[],
          number:[]
        },
        resumeState: []
      }
    }, created () {
      this.init()
    },
    mounted () {
      this.companyLocationEchart()
      this.companyLocationListEchart()
      this.companyNewProfessionEchart()
      this.courseProfessionListEchart()
      this.courseResumeListEchart()
      this.courseResumeStateEchart()
    },
    methods: {
      //初始化
      init () {
        //公司地理分布情况
        this.$axios.post(`/companyApi/service.v1.WorknetCompany/WorknetLocationList`).then(res => {
          if (res.data.code === 0) {
            this.locationList = res.data.data.locationList
            this.companyLocationEchart()
            this.companyLocationListEchart()
          }
        })

        //公司列表
        this.$axios.post(`/companyApi/service.v1.WorknetCompany/WorknetCompanyList`).then(res => {
          if (res.data.code === 0) {
            this.companyOptions = res.data.data.companyList
          }
        })

        //公司发布招聘数量
        this.$axios.post(`/companyApi/service.v1.WorknetCompany/WorknetProfessionNumberRank`).then(res => {
          if (res.data.code === 0) {
            this.professionList = res.data.data
            this.courseProfessionListEchart()
          }
        })

        //公司收到的简历数量
        this.$axios.post(`/companyApi/service.v1.WorknetCompany/WorknetCvNumberRank`).then(res => {
          if (res.data.code === 0) {
            this.resumeList = res.data.data
            this.courseResumeListEchart()
          }
        })

        //公司收到的简历状态
        this.$axios.post(`/companyApi/service.v1.WorknetCompany/WorknetCvStatusNumber`).then(res => {
          if (res.data.code === 0) {
            this.resumeState = res.data.data.statusNumber
            this.courseResumeStateEchart()
          }
        })

        //默认一年的区间变化
        const start = this.$moment().subtract(1, 'years').unix()//一年前的日期
        const end = this.$moment().unix()                       //当前的日期
        this.selectValue.timeValue.push(start * 1000)
        this.selectValue.timeValue.push(end * 1000)
      },

      //地区公司分布数量情况
      companyLocationEchart () {
        var myChart = this.$echarts.init(document.getElementById('company_location_echart'))

        var data = this.locationList
        var max = 0
        var maxData = data.sort(function (a, b) {
          return b.value - a.value
        }).slice(0, 1)[0]
        if (maxData !== undefined && maxData !== null) {
          max = maxData.value
        }
        var geoCoordMap = {
          '海门': [121.15, 31.89],
          '鄂尔多斯': [109.781327, 39.608266],
          '招远': [120.38, 37.35],
          '舟山': [122.207216, 29.985295],
          '齐齐哈尔': [123.97, 47.33],
          '盐城': [120.13, 33.38],
          '赤峰': [118.87, 42.28],
          '青岛': [120.33, 36.07],
          '乳山': [121.52, 36.89],
          '金昌': [102.188043, 38.520089],
          '泉州': [118.58, 24.93],
          '莱西': [120.53, 36.86],
          '日照': [119.46, 35.42],
          '胶南': [119.97, 35.88],
          '南通': [121.05, 32.08],
          '拉萨': [91.11, 29.97],
          '云浮': [112.02, 22.93],
          '梅州': [116.1, 24.55],
          '文登': [122.05, 37.2],
          '上海': [121.48, 31.22],
          '攀枝花': [101.718637, 26.582347],
          '威海': [122.1, 37.5],
          '承德': [117.93, 40.97],
          '厦门': [118.1, 24.46],
          '汕尾': [115.375279, 22.786211],
          '潮州': [116.63, 23.68],
          '丹东': [124.37, 40.13],
          '太仓': [121.1, 31.45],
          '曲靖': [103.79, 25.51],
          '烟台': [121.39, 37.52],
          '福州': [119.3, 26.08],
          '瓦房店': [121.979603, 39.627114],
          '即墨': [120.45, 36.38],
          '抚顺': [123.97, 41.97],
          '玉溪': [102.52, 24.35],
          '张家口': [114.87, 40.82],
          '阳泉': [113.57, 37.85],
          '莱州': [119.942327, 37.177017],
          '湖州': [120.1, 30.86],
          '汕头': [116.69, 23.39],
          '昆山': [120.95, 31.39],
          '宁波': [121.56, 29.86],
          '湛江': [110.359377, 21.270708],
          '揭阳': [116.35, 23.55],
          '荣成': [122.41, 37.16],
          '连云港': [119.16, 34.59],
          '葫芦岛': [120.836932, 40.711052],
          '常熟': [120.74, 31.64],
          '东莞': [113.75, 23.04],
          '河源': [114.68, 23.73],
          '淮安': [119.15, 33.5],
          '泰州': [119.9, 32.49],
          '南宁': [108.33, 22.84],
          '营口': [122.18, 40.65],
          '惠州': [114.4, 23.09],
          '江阴': [120.26, 31.91],
          '蓬莱': [120.75, 37.8],
          '韶关': [113.62, 24.84],
          '嘉峪关': [98.289152, 39.77313],
          '广州': [113.23, 23.16],
          '延安': [109.47, 36.6],
          '太原': [112.53, 37.87],
          '清远': [113.01, 23.7],
          '中山': [113.38, 22.52],
          '昆明': [102.73, 25.04],
          '寿光': [118.73, 36.86],
          '盘锦': [122.070714, 41.119997],
          '长治': [113.08, 36.18],
          '深圳': [114.07, 22.62],
          '珠海': [113.52, 22.3],
          '宿迁': [118.3, 33.96],
          '咸阳': [108.72, 34.36],
          '铜川': [109.11, 35.09],
          '平度': [119.97, 36.77],
          '佛山': [113.11, 23.05],
          '海口': [110.35, 20.02],
          '江门': [113.06, 22.61],
          '章丘': [117.53, 36.72],
          '肇庆': [112.44, 23.05],
          '大连': [121.62, 38.92],
          '临汾': [111.5, 36.08],
          '吴江': [120.63, 31.16],
          '石嘴山': [106.39, 39.04],
          '沈阳': [123.38, 41.8],
          '苏州': [120.62, 31.32],
          '茂名': [110.88, 21.68],
          '嘉兴': [120.76, 30.77],
          '长春': [125.35, 43.88],
          '胶州': [120.03336, 36.264622],
          '银川': [106.27, 38.47],
          '张家港': [120.555821, 31.875428],
          '三门峡': [111.19, 34.76],
          '锦州': [121.15, 41.13],
          '南昌': [115.89, 28.68],
          '柳州': [109.4, 24.33],
          '三亚': [109.511909, 18.252847],
          '自贡': [104.778442, 29.33903],
          '吉林': [126.57, 43.87],
          '阳江': [111.95, 21.85],
          '泸州': [105.39, 28.91],
          '西宁': [101.74, 36.56],
          '宜宾': [104.56, 29.77],
          '呼和浩特': [111.65, 40.82],
          '成都': [104.06, 30.67],
          '大同': [113.3, 40.12],
          '镇江': [119.44, 32.2],
          '桂林': [110.28, 25.29],
          '张家界': [110.479191, 29.117096],
          '宜兴': [119.82, 31.36],
          '北海': [109.12, 21.49],
          '西安': [108.95, 34.27],
          '金坛': [119.56, 31.74],
          '东营': [118.49, 37.46],
          '牡丹江': [129.58, 44.6],
          '遵义': [106.9, 27.7],
          '绍兴': [120.58, 30.01],
          '扬州': [119.42, 32.39],
          '常州': [119.95, 31.79],
          '潍坊': [119.1, 36.62],
          '重庆': [106.54, 29.59],
          '台州': [121.420757, 28.656386],
          '南京': [118.78, 32.04],
          '滨州': [118.03, 37.36],
          '贵阳': [106.71, 26.57],
          '无锡': [120.29, 31.59],
          '本溪': [123.73, 41.3],
          '克拉玛依': [84.77, 45.59],
          '渭南': [109.5, 34.52],
          '马鞍山': [118.48, 31.56],
          '宝鸡': [107.15, 34.38],
          '焦作': [113.21, 35.24],
          '句容': [119.16, 31.95],
          '北京': [116.46, 39.92],
          '徐州': [117.2, 34.26],
          '衡水': [115.72, 37.72],
          '包头': [110, 40.58],
          '绵阳': [104.73, 31.48],
          '乌鲁木齐': [87.68, 43.77],
          '枣庄': [117.57, 34.86],
          '杭州': [120.19, 30.26],
          '淄博': [118.05, 36.78],
          '鞍山': [122.85, 41.12],
          '溧阳': [119.48, 31.43],
          '库尔勒': [86.06, 41.68],
          '安阳': [114.35, 36.1],
          '开封': [114.35, 34.79],
          '济南': [117, 36.65],
          '德阳': [104.37, 31.13],
          '温州': [120.65, 28.01],
          '九江': [115.97, 29.71],
          '邯郸': [114.47, 36.6],
          '临安': [119.72, 30.23],
          '兰州': [103.73, 36.03],
          '沧州': [116.83, 38.33],
          '临沂': [118.35, 35.05],
          '南充': [106.110698, 30.837793],
          '天津': [117.2, 39.13],
          '富阳': [119.95, 30.07],
          '泰安': [117.13, 36.18],
          '诸暨': [120.23, 29.71],
          '郑州': [113.65, 34.76],
          '哈尔滨': [126.63, 45.75],
          '聊城': [115.97, 36.45],
          '芜湖': [118.38, 31.33],
          '唐山': [118.02, 39.63],
          '平顶山': [113.29, 33.75],
          '邢台': [114.48, 37.05],
          '德州': [116.29, 37.45],
          '济宁': [116.59, 35.38],
          '荆州': [112.239741, 30.335165],
          '宜昌': [111.3, 30.7],
          '义乌': [120.06, 29.32],
          '丽水': [119.92, 28.45],
          '洛阳': [112.44, 34.7],
          '秦皇岛': [119.57, 39.95],
          '株洲': [113.16, 27.83],
          '石家庄': [114.48, 38.03],
          '莱芜': [117.67, 36.19],
          '常德': [111.69, 29.05],
          '保定': [115.48, 38.85],
          '湘潭': [112.91, 27.87],
          '金华': [119.64, 29.12],
          '岳阳': [113.09, 29.37],
          '长沙': [113, 28.21],
          '衢州': [118.88, 28.97],
          '廊坊': [116.7, 39.53],
          '菏泽': [115.480656, 35.23375],
          '合肥': [117.27, 31.86],
          '武汉': [114.31, 30.52],
          '大庆': [125.03, 46.58]
        }
        var convertData = function (data) {
          var res = []
          for (var i = 0; i < data.length; i++) {
            var geoCoord = geoCoordMap[data[i].name]
            if (geoCoord) {
              res.push({
                name: data[i].name,
                value: geoCoord.concat(data[i].value)
              })
            }
          }
          return res
        }

        var option = {
          title: {
            text: '全国互联网公司分布地图',
            subtext: 'worknet',
            sublink: 'http://www.jmedal.in',
            left: 'center'
          },
          tooltip: {
            formatter: function (obj) {
              var relVal = ''
              relVal += obj.seriesName + '<br/>'
                + obj.name + ' : ' + obj.value[2] + ' 家公司'
              return relVal
            },
            trigger: 'item'
          },
          bmap: {
            center: [104.114129, 37.550339],
            zoom: 5,
            roam: true,
            mapStyle: {
              styleJson: [{
                'featureType': 'water',
                'elementType': 'all',
                'stylers': {
                  'color': '#d1d1d1'
                }
              }, {
                'featureType': 'land',
                'elementType': 'all',
                'stylers': {
                  'color': '#f3f3f3'
                }
              }, {
                'featureType': 'railway',
                'elementType': 'all',
                'stylers': {
                  'visibility': 'off'
                }
              }, {
                'featureType': 'highway',
                'elementType': 'all',
                'stylers': {
                  'color': '#fdfdfd'
                }
              }, {
                'featureType': 'highway',
                'elementType': 'labels',
                'stylers': {
                  'visibility': 'off'
                }
              }, {
                'featureType': 'arterial',
                'elementType': 'geometry',
                'stylers': {
                  'color': '#fefefe'
                }
              }, {
                'featureType': 'arterial',
                'elementType': 'geometry.fill',
                'stylers': {
                  'color': '#fefefe'
                }
              }, {
                'featureType': 'poi',
                'elementType': 'all',
                'stylers': {
                  'visibility': 'off'
                }
              }, {
                'featureType': 'green',
                'elementType': 'all',
                'stylers': {
                  'visibility': 'off'
                }
              }, {
                'featureType': 'subway',
                'elementType': 'all',
                'stylers': {
                  'visibility': 'off'
                }
              }, {
                'featureType': 'manmade',
                'elementType': 'all',
                'stylers': {
                  'color': '#d1d1d1'
                }
              }, {
                'featureType': 'local',
                'elementType': 'all',
                'stylers': {
                  'color': '#d1d1d1'
                }
              }, {
                'featureType': 'arterial',
                'elementType': 'labels',
                'stylers': {
                  'visibility': 'off'
                }
              }, {
                'featureType': 'boundary',
                'elementType': 'all',
                'stylers': {
                  'color': '#fefefe'
                }
              }, {
                'featureType': 'building',
                'elementType': 'all',
                'stylers': {
                  'color': '#d1d1d1'
                }
              }, {
                'featureType': 'label',
                'elementType': 'labels.text.fill',
                'stylers': {
                  'color': '#999999'
                }
              }]
            }
          },
          series: [
            {
              name: '公司数量',
              type: 'scatter',
              coordinateSystem: 'bmap',
              data: convertData(data),
              symbolSize: function (val) {
                return 35 * val[2] / max
              },
              label: {
                formatter: '{b}',
                position: 'right',
                show: false
              },
              itemStyle: {
                color: '#f3d999'
              },
              emphasis: {
                label: {
                  show: true
                }
              }
            },
            {
              name: 'Top 5',
              type: 'effectScatter',
              coordinateSystem: 'bmap',
              data: convertData(data.sort(function (a, b) {
                return b.value - a.value
              }).slice(0, 5)),
              symbolSize: function (val) {
                return 35 * val[2] / max
              },
              showEffectOn: 'render',
              rippleEffect: {
                brushType: 'stroke'
              },
              hoverAnimation: true,
              label: {
                formatter: '{b}',
                position: 'right',
                show: true
              },
              itemStyle: {
                color: '#f5e8c8',
                shadowBlur: 10,
                shadowColor: '#333'
              },
              zlevel: 1
            }
          ]
        }

        myChart.setOption(option, true)
      },

      //地区公司分布数量排名
      companyLocationListEchart () {
        var myChart = this.$echarts.init(document.getElementById('company_location_list_echart'), 'westeros')
        var topData = this.locationList.sort(function (a, b) {
          return b.value - a.value
        })
        if (topData.length > 15) {
          topData = topData.slice(0, 15)
        }
        var name = [], num = []
        for (var i = 0; i < topData.length; i++) {
          name.unshift('Top' + (i + 1) + ':' + topData[i].name)
          num.unshift(topData[i].value)
        }
        var option = {
          title: {
            text: '排名Top12',
          },
          tooltip: {
            trigger: 'axis',
            axisPointer: {
              type: 'shadow'
            }
          },
          color: ['#f3d999'],
          legend: {
            data: ['公司数量'],
            left: 'right'
          },
          grid: {
            left: '3%',
            right: '4%',
            bottom: '3%',
            containLabel: true
          },
          xAxis: {
            type: 'value',
            boundaryGap: [0, 0.01]
          },
          yAxis: {
            type: 'category',
            data: name
          },
          series: [
            {
              name: '公司数量',
              type: 'bar',
              data: num
            },
          ]
        }
        myChart.setOption(option, true)
      },

      //公司发布招聘数量变化
      companyNewProfessionEchart () {
        var myChart = this.$echarts.init(document.getElementById('company_new_profession_echart'), 'westeros')
        var companyName = []
        var seriesDate = []
        if (this.professionChange.profession === undefined || this.professionChange.profession == null) {
          this.professionChange.profession = []
        }
        for (var i = 0; i < this.professionChange.profession.length; i++) {
          companyName.push(this.professionChange.profession[i].companyName)
          var option = {
            name: this.professionChange.profession[i].companyName,
            type: 'line',
            stack: this.professionChange.profession[i].companyName,
            areaStyle: {},
            data: this.professionChange.profession[i].professionChange
          }
          seriesDate.push(option)
        }
        var option = {
          title: {
            text: '公司月发布招聘数量变化趋势'
          },
          tooltip: {
            trigger: 'axis',
            axisPointer: {
              type: 'cross',
              label: {
                backgroundColor: '#6a7985'
              }
            }
          },
          legend: {
            data: companyName
          },
          toolbox: {
            show: true,
            feature: {
              saveAsImage: {}
            }
          },
          grid: {
            left: '3%',
            right: '4%',
            bottom: '3%',
            containLabel: true
          },
          xAxis: [
            {
              type: 'category',
              boundaryGap: false,
              data: this.professionChange.time
            }
          ],
          yAxis: {
            type: 'value',
            name: '招聘',
            axisLabel: {
              formatter: '{value} 篇'
            }
          },
          series: seriesDate
        }
        myChart.setOption(option, true)
      },

      //标签删除
      handleClose (tag) {
        var tagIndex = this.tags.indexOf(tag)
        var courseIdIndex = this.selectValue.companyId.indexOf(this.tags[tagIndex].id)
        this.selectValue.companyId.splice(courseIdIndex, 1)
        this.tags.splice(tagIndex, 1)
      },

      //公司招聘数量排行
      courseProfessionListEchart () {
        var myChart = this.$echarts.init(document.getElementById('company_profession_list_echart'), 'westeros')
        var option = {
          title: {
            text: '公司发布招聘数量Top10',
            subtext: 'worknet'
          },
          tooltip: {
            trigger: 'axis',
            axisPointer: {
              type: 'shadow'
            }
          },
          toolbox: {
            show: true,
            top: 20,
            feature: {
              saveAsImage: {}
            }
          },
          color: ['#59c4e6'],
          legend: {
            data: ['招聘数量'],
            left: 'right'
          },
          grid: {
            left: '3%',
            right: '4%',
            bottom: '3%',
            containLabel: true
          },
          xAxis: {
            type: 'value',
            boundaryGap: [0, 0.01]
          },
          yAxis: {
            type: 'category',
            data: this.professionList.companyName.reverse()
          },
          series: [
            {
              name: '招聘数量',
              type: 'bar',
              data: this.professionList.number.reverse()
            },
          ]
        }
        myChart.setOption(option, true)
      },

      //公司简历数量排行
      courseResumeListEchart () {
        var myChart = this.$echarts.init(document.getElementById('company_resume_list_echart'), 'westeros')
        var option = {
          title: {
            text: '公司收到简历数量Top10',
            subtext: 'worknet'
          },
          tooltip: {
            trigger: 'axis',
            axisPointer: {
              type: 'shadow'
            }
          },
          toolbox: {
            show: true,
            top: 20,
            feature: {
              saveAsImage: {}
            }
          },
          color: ['#edafda'],
          legend: {
            data: ['简历数量'],
            left: 'right'
          },
          grid: {
            left: '3%',
            right: '4%',
            bottom: '3%',
            containLabel: true
          },
          xAxis: {
            type: 'value',
            boundaryGap: [0, 0.01]
          },
          yAxis: {
            type: 'category',
            data: this.resumeList.companyName.reverse()
          },
          series: [
            {
              name: '简历数量',
              type: 'bar',
              data: this.resumeList.number.reverse()
            },
          ]
        }
        myChart.setOption(option, true)
      },

      //公司简历状态比例
      courseResumeStateEchart () {
        var myChart = this.$echarts.init(document.getElementById('company_resume_state_echart'), 'westeros')
        var option = {
          title: {
            text: '公司所收简历状态比例',
            subtext: 'worknet',
            left: 'center'
          },
          tooltip: {
            trigger: 'item',
            formatter: '{a} <br/>{b} : {c} ({d}%)'
          },
          toolbox: {
            show: true,
            feature: {
              saveAsImage: {}
            }
          },
          legend: {
            orient: 'vertical',
            left: 'left',
            data: ['待审核', '审核中', '拒绝', '接受']
          },
          series: [
            {
              name: '数量',
              type: 'pie',
              radius: '55%',
              center: ['50%', '60%'],
              data: [
                {value: this.resumeState[0], name: '待审核'},
                {value: this.resumeState[1], name: '审核中'},
                {value: this.resumeState[2], name: '拒绝'},
                {value: this.resumeState[3], name: '接受'},
              ],
              emphasis: {
                itemStyle: {
                  shadowBlur: 10,
                  shadowOffsetX: 0,
                  shadowColor: 'rgba(0, 0, 0, 0.5)'
                }
              }
            }
          ]
        }
        myChart.setOption(option, true)
      }
    },
    watch: {
      //公司和时间监控，加载公司招聘数量变化
      selectValue: {
        handler (newVal, oldVal) {
          this.$axios.post(`/companyApi/service.v1.WorknetCompany/WorknetCompanyProfessionChange`, {
            companyId: newVal.companyId,
            time: [newVal.timeValue[0] / 1000, newVal.timeValue[1] / 1000]
          }).then(res => {
            if (res.data.code === 0) {
              this.professionChange = res.data.data
              this.companyNewProfessionEchart()
            }
          })
        },
        deep: true,
      },

      //选择公司，添加标签
      companyValue: {
        handler (newVal, oldVal) {
          if (newVal !== undefined && newVal !== null && newVal !== '' && !this.selectValue.companyId.includes(newVal)) {
            this.selectValue.companyId.push(newVal)
            this.tags.push({id: newVal, name: this.companyOptions.find(item => item.companyId === newVal).companyName})
          }
        },
        deep: true,
      }
    }
  }
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style lang="less" scoped>
  .company_container {
    height: 100%;
  }

  .company_up_box {
    height: 550px;
    width: 100%;
    background-color: #fff;
    border-radius: 3px;
    box-shadow: 0 0 5px #ddd;
  }

  .company_data_title {
    height: 30px;
    background-color: #fff;
    border-radius: 3px;
    border-left: 5px solid #59c4e6;
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 12pt;
  }

  .company_location_box {
    height: 480px;
    border-radius: 3px;
    padding: 5px;
    box-shadow: 0 0 5px #ddd;
  }

  .company_location_list_box {
    height: 480px;
    border-radius: 3px;
    padding: 5px;
    box-shadow: 0 0 5px #ddd;
  }

  .company_center_box {
    height: 670px;
    width: 100%;
    background-color: #fff;
    border-radius: 3px;
    box-shadow: 0 0 10px #ddd;
    position: relative;
    left: 0;
    top: 10px;
  }

  .company_select_box {
    height: 210px;
    width: 100%;
  }

  .company_time_select_options {
    height: 42px;
    display: flex;
    align-items: center;
    justify-content: left;
    font-size: 12pt;
  }

  .company_select_options {
    height: 42px;
    display: flex;
    align-items: center;
    justify-content: left;
    font-size: 12pt;
  }

  .company_new_profession_box {
    height: 400px;
    width: 100%;
    border-radius: 3px;
  }

  .company_new_profession_line_box {
    height: 450px;
    display: flex;
    align-items: center;
    justify-content: center;
  }

  .company_end_box {
    height: 495px;
    width: 100%;
    background-color: #fff;
    border-radius: 3px;
    box-shadow: 0 0 10px #ddd;
    position: relative;
    left: 0;
    top: 20px;
  }

  .company_list_box {
    height: 450px;
    padding: 5px;
  }

  .el-row {
    margin-bottom: 15px;

    &:last-child {
      margin-bottom: 0px;
    }
  }

  .el-col {
    border-right: 3px solid #f0f0f0;

    &:last-child {
      border-right: 0;
    }
  }
</style>
